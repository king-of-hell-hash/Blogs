...
import { getCuratedImageUrl } from './_imageFallback';
...
export async function executeGenerationWithFallbacks(
  ai: GoogleGenAI,
  params: {
    prompt: string;
    responseSchema: any;
...
export async function handleGenerateBlogPost(body: any): Promise<{ status: number; data: any }> {
  ...
  if (!keyword) {
    return {
      status: 400,
      data: { error: "Keyword or Topic is required" }
    };
  }
...
export async function handleGenerateImage(body: any): Promise<{ status: number; data: any }> {
  const apiKey = getCleanApiKey();
  if (!apiKey) {
    const fallbackUrl = getCuratedImageUrl(body?.prompt || 'technology', body?.aspectRatio || '16:9');
    return {
      status: 200,
      data: {
        image: fallbackUrl,
        isFallback: true,
        fallbackReason: "Curated HD visual provided."
      }
    };
  }
  ...
  const { prompt, aspectRatio = '16:9', style = 'photorealistic', placementOffset = 0 } = body || {};

  if (!prompt) {
    return {
      status: 400,
      data: { error: "Image prompt is required" }
    };
  }
